...
void main() {
  group('BuildVu converter screen widget test', () {
    ...
    testWidgets(
        'test if clicking on convert btn without selecting a file, a snackbar shows up',
        (tester) async {
      await tester.pumpWidget(createHomeScreen());
      expect(find.byType(ColorfulBgBtn), findsOneWidget);
    ...
    testWidgets(
        'test if pdf password text field shows up and the input is obscure',
        (tester) async {
      await tester.pumpWidget(createHomeScreen());
      ...
      final textFieldWidget =
          tester.widget<RectangleTextField>(find.byKey(Key('passwordField')));
      expect(textFieldWidget.isObscureText, isTrue);
    });

    testWidgets(
        'test if img scale text field shows up and typing in non-number triggers snackbar',
        (tester) async {
      await tester.pumpWidget(createHomeScreen());
    ...
    testWidgets(
        'test if type in off-limit number in img scale text field, the number changes according to logic',
        (tester) async {
      await tester.pumpWidget(createHomeScreen());
      ...
      final textField = tester
          .widget<RectangleTextField>(find.byKey(Key('imgScaleTextField')));
      expect(textField.controller.text, '10.0');
    });

    testWidgets('test if idr ui dropdown shows up and is clickable',
        (tester) async {
      await tester.pumpWidget(createHomeScreen());
    ...
    testWidgets('test if text mode shows up and is clickable', (tester) async {
      await tester.pumpWidget(createHomeScreen());
    ...
    testWidgets('test if embed checkbox and inline checkbox show up',
        (tester) async {
      await tester.pumpWidget(createHomeScreen());
